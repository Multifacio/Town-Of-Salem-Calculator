...
# Likelihood After Night 5, so only the event during night 5 are used as evidence (which includes the players that
# died during this night, but does not include the role of the player that is lynched during day 5). The claims that
# will be tested are the claims made during day 5.

# We assumed that 5 really was Sheriff in the evidence and that his will was correct.
condition = ANDCondition([DeathRole(1, Role.MEDIUM), OtherSingleVisitHarmless(3, 7, False), ActualRole(9, Role.SHERIFF),
    ...
    DeathRole(13, Role.GODFATHER), ActualRole(5, Role.SHERIFF), NotSuspicious(12, False)])
...
analyzer.add_event("A Lesbian claims Veteran", ActualRole(6, Role.VETERAN))
...
analyzer.compute_likelihoods("After Night 5")